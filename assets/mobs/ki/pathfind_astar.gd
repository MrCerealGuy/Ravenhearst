...
func _init(map, map_size):
	self.map = map
	self.map_size = map_size
	
	astar_node = AStar.new()
	
	obstacles = map.get_used_cells_by_id(0)
	_half_cell_size = map.cell_size / 2
...
func _get_path(world_start, world_end):
	...
	for point in _point_path:
		var point_world = map.map_to_world(Vector2(point.x, point.y)) + _half_cell_size
...
func draw():
	...
	var last_point = map.map_to_world(Vector2(point_start.x, point_start.y)) + _half_cell_size
	for index in range(1, len(_point_path)):
		var current_point = map.map_to_world(Vector2(_point_path[index].x, _point_path[index].y)) + _half_cell_size
		map.draw_line(last_point, current_point, DRAW_COLOR, BASE_LINE_WIDTH, true)
		map.draw_circle(current_point, BASE_LINE_WIDTH * 2.0, DRAW_COLOR)
